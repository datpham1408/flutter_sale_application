...
class _ChatGroupScreenState extends State<ChatGroupScreen> {
          ...
          (snapshot) {
            for (var change in snapshot.docChanges) {
              ...
              setState(() {
                ...
                timestamps.add(_chatGroupCubit.formatTime(chatMessage.timestamp));
              });
            }
  ...
  Future<void> _sendMessage(String message) async {
    if (userEntity.fullName == widget.entity?.fullName) {
      final chatMessage = ChatMessage(
        message: message,
        sender: userEntity.fullName ??'',
  ...
  void _sortMessagesByTimestamp() {
    ...
    timestamps = _chatMessages.map((message) => _chatGroupCubit.formatTime(message.timestamp)).toList();
  }
